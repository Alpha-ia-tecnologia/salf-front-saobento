...
btnFiltrar.addEventListener('click', async (e) => {
    ...
    const response = await fetch(`https://salf-salf-api2.gkgtsp.easypanel.host/api/dashboard/school-ranking?region=${regiao}&group=${grupo}&school=${escola}&event=${evento}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
