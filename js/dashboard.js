 ...
 * FUNÇÃO: Dashboard principal com métricas e gráficos
 *
 * Este arquivo gerencia o painel principal do sistema:
 * - Carregamento de dados de analytics da API
 ...
 *
 * RELACIONAMENTOS:
 * - Integra com Chart.js para visualizações
 * - Consome endpoints de dashboard da API
 * - Utiliza sistema de cache para otimização
 * - Conecta com ExportFile.js para exportação
 */
...
async function initialize() {
  ...
  PopularGraphYear(response3.yearly);
  const data4 = await fetch(
    API_BASE_URL + "/dashboard/reading-level-evolution",
    {
      headers: headers,
    }
  );
...
const filterClass = async () => {
  if (escola.value) {
    ...
    const request = await fetch(
      API_BASE_URL + `/class-groups?schoolId=${escola.value}&limit=1000`,
      {
        headers: headers,
      }
    );
