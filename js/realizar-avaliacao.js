 ...
 *
 * ARQUIVO: realizar-avaliacao.js
 * FUNÇÃO: Sistema de realização de avaliações de leitura
 *
 * Este arquivo gerencia o processo de aplicação das avaliações:
 * - Seleção de aluno, turma e evento de avaliação
 * - Navegação entre etapas sequenciais de leitura
 * - Controle de timers e cronômetros por etapa
 * - Coleta de dados de desempenho em tempo real
 * - Cálculo automático de PPM e níveis de leitura
 *
 * RELACIONAMENTOS:
 * - Consome dados de escolas, turmas e alunos da API
 * - Integra com sistema de avaliações para carregar testes
 * - Conecta com leituras.js para etapas de leitura
 * - Persiste resultados na API e localStorage
 */
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const elementos = {
    ...
    iniciarTimerPseudopalavrasBtn: document.getElementById(
    ...
    proximoEtapaPseudopalavrasBtn: document.getElementById(
  ...
  for (const { elemento, nome } of elementosEssenciais) {
    if (!elemento) {
      console.error(
        `ERRO CRÍTICO: Elemento '${nome}' não encontrado na página!`
      );
  ...
  if (!todosElementosEncontrados) {
    alert(
      "Erro: O formulário não pode ser inicializado corretamente. Por favor, recarregue a página."
    );
    return;
  }
  ...
  function carregarEscolas() {
    ...
    elementos.escolaSelect.innerHTML =
  ...
  function carregarTodosAlunos() {
    ...
    elementos.alunoSelect.innerHTML =
    ...
    fetch(`${API_BASE_URL}/students`, { headers })
      ...
      .then((data) => {
        ...
        alunos.forEach((aluno) => {
          ...
          option.textContent = `${aluno.name || aluno.nome} (${
            aluno.registration || aluno.matricula || "N/A"
          })`;
      ...
      .catch((error) => {
        ...
        const alunosEstaticos = [
          ...
          { id: 3, nome: "Pedro Oliveira", matricula: "2023003" },
        ];
  ...
  function carregarTurmasDaEscola(escolaId) {
    ...
    elementos.turmaSelect.innerHTML =
    ...
    fetch(`${API_BASE_URL}/classes/${turmaId}/students`, { headers })
      ...
      .then((data) => {
        ...
        alunos.forEach((aluno) => {
          ...
          option.value = aluno.id;
          option.textContent = `${aluno.name || aluno.nome} (${
            aluno.registrationNumber || aluno.matricula || "N/A"
          })`;
          elementos.alunoSelect.appendChild(option);
        });
  ...
  function carregarEventosAvaliacao() {
    ...
    elementos.eventoSelect.innerHTML =
  ...
  function determinarFaixaSerie(turmaId) {
    fetch(`${API_BASE_URL}/classes/${turmaId}`, { headers })
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Erro ao buscar detalhes da turma: ${response.status}`
          );
        }
        return response.json();
      })
      .then((turma) => {
        ...
        console.log(
          `Faixa de série determinada: ${estado.faixaSerie} (Série: ${serie}, Número: ${numeroSerie})`
        );
      })
      .catch((error) => {
  ...
  function carregarDetalhesAluno(alunoId) {
    fetch(`${API_BASE_URL}/students/${alunoId}`, { headers })
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Erro ao buscar detalhes do aluno: ${response.status}`
          );
        }
        return response.json();
      })
      .then((aluno) => {
        ...
        if (
          turmaAluno &&
          turmaAluno !== parseInt(elementos.turmaSelect.value)
        ) {
  ...
  function iniciarAvaliacao() {
    ...
    if (!alunoId || !eventoId) {
      alert(
        "Por favor, selecione um aluno e um evento de avaliação para continuar."
      );
      return;
    }

    console.log(
      `Iniciando avaliação: Aluno ID ${alunoId}, Evento ID ${eventoId}`
    );

    obterProvaCorrespondente().then((prova) => {
      if (!prova) {
        alert(
          "Não foi possível encontrar uma prova adequada para o aluno selecionado."
        );
        return;
      }
      ...
      buscarDetalhesAvaliacao(eventoId, prova.id)
        ...
        .catch((error) => {
          ...
          alert(
            "Ocorreu um erro ao iniciar a avaliação. Por favor, tente novamente."
          );
        });
    });
  }
      ...
      fetch(`${API_BASE_URL}/reading-tests`, { headers })
        ...
        .then((data) => {
          ...
          if (provaCorrespondente) {
            ...
            console.warn(
              "Nenhuma prova encontrada para a faixa de série:",
              estado.faixaSerie
            );
            ...
            const provasEstaticas = [
              ...
              {
                id: 2,
                nome: "Prova de Leitura - Intermediário",
                niveis: ["3-5"],
              },
  ...
  function buscarDetalhesAvaliacao(eventoId, provaId) {
    return new Promise((resolve, reject) => {
      fetch(`${API_BASE_URL}/assessment-events/${eventoId}/tests/${provaId}`, {
        headers,
      })
        .then((response) => {
          if (!response.ok) {
            throw new Error(
              `Erro ao buscar detalhes da avaliação: ${response.status}`
            );
          }
          return response.json();
        })
        ...
        .catch((error) => {
          ...
          const avaliacaoEstatica = avaliacoesEstaticas.find(
            (a) => a.id === eventoId
          );
