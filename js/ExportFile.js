 ...
 *
 * ARQUIVO: ExportFile.js
 * FUNÇÃO: Sistema de exportação de dados para Excel
 *
 * Este arquivo gerencia a exportação de dados do dashboard:
 * - Coleta dados de múltiplos endpoints da API
 * - Processa e formata dados para exportação
 * - Gera arquivos Excel com múltiplas abas
 * - Traduz campos para português
 * - Suporta filtros aplicados no dashboard
 *
 * RELACIONAMENTOS:
 * - Integra com dashboard.js para filtros aplicados
 * - Consome múltiplos endpoints de analytics da API
 * - Utiliza biblioteca XLSX para geração de arquivos
 * - Conecta com sistema de autenticação para requisições
 */
...
const Dashboard = async () => {
  ...
  const perfomanceByGradeRequest = await fetch(
    url.concat(
      "/performance-by-grade?schoolId=" +
        schoolIdExp.value +
        "&eventId=" +
        eventIdExp.value
    ),
  ...
  const performanceByEventRequest = await fetch(
    url.concat(
      "/reading-level-evolution?schoolId=" +
        schoolIdExp.value +
        "&eventId=" +
        eventIdExp.value
    ),
  ...
  const readingLevelDistributionRequest = await fetch(
    url.concat(
      "/reading-level-distribution?schoolId=" +
        schoolIdExp.value +
        "&eventId=" +
        eventIdExp.value +
        "&gradeLevel=" +
  ...
  const yearlyProgressionRequest = await fetch(
    url.concat(
      "/yearly-progression?schoolId=" +
        schoolIdExp.value +
        "&grupoId=" +
        grupoIdExp.value +
        "&gradeLevel=" +
  ...
  const rankingsSchoolRequest = await fetch(
    url.concat(
      "/school-ranking?schoolId=" +
        schoolIdExp.value +
        "&grupoId=" +
        grupoIdExp.value +
        "&gradeLevel=" +
  ...
  const rakingAlunoRequest = await fetch(
    url.concat(
      "/student-ranking?schoolId=" +
        schoolIdExp.value +
        "&grupoId=" +
        grupoIdExp.value +
        "&gradeLevel=" +
  ...
  const rankingRegionRequest = await fetch(
    url.concat(
      "/ranking-by-region?schoolId=" +
        schoolIdExp.value +
        "&grupoId=" +
        grupoIdExp.value +
        "&gradeLevel=" +
  ...
  const objectPerfomanceByGrade = transformarDistribuicaoEmTabela(
    perfomanceByGrade.gradePerformance,
    "grade"
  );
  const objectPerformanceByEvent = transformarDistribuicaoEmTabela(
    performanceByEvent.evolution,
    "event"
  );
  ...
  const objectReadingLevelDistribution =
    readingLevelDistribution.distribution.map((item) => {
      return {
        nivel: item.name,
  ...
  objectYearlyProgression = traduzirCabecalhos(
    objectYearlyProgression,
    traducoes
  );
...
function transformarDistribuicaoEmTabela(dados, type) {
  ...
  const colunas = [
    ...new Set(dados.flatMap((item) => item.distribution.map((d) => d.name))),
  ];

  // Agora, montar cada linha
  const tabela = dados.map((item) => {
    const linha =
      type === "grade" ? { Série: item.grade } : { Evento: item.eventName };
