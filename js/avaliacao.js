 ...
 * FUNÇÃO: Sistema de gestão de avaliações e eventos
 *
 * Este arquivo gerencia todo o ciclo de vida das avaliações:
 * - CRUD completo de avaliações (criar, ler, atualizar, deletar)
 * - Gestão de eventos de avaliação
 * - Sistema de palavras, pseudopalavras e frases
 * - Questões de múltipla escolha para compreensão
 * - Validação de formulários e dados
 *
 * RELACIONAMENTOS:
 * - Integra com API de assessments e assessment-events
 * - Conecta com sistema de roles para permissões
 * - Fornece dados para realização de avaliações
 * - Gerencia estrutura hierárquica de avaliações
 */
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnNovaAvaliacao) {
    btnNovaAvaliacao.addEventListener("click", () => {
      ...
      resetFormAvaliacao();
      document.querySelector("#modal-avaliacao h3").textContent =
        "Nova Avaliação";
...
const btnNovaAvaliacao = document.getElementById("btn-nova-avaliacao");
const btnExportarAvaliacoes = document.getElementById(
  "btn-exportar-avaliacoes"
);
...
const adicionarPseudopalavra = document.getElementById(
  "adicionar-pseudopalavra"
);
...
const fecharModalEditarAvaliacao = document.getElementById(
  "fechar-modal-editar-avaliacao"
);
const cancelarEditarAvaliacao = document.getElementById(
  "cancelar-editar-avaliacao"
);
const formEditarAvaliacao = document.getElementById("form-editar-avaliacao");
const btnSalvarEditarAvaliacao = document.getElementById(
  "btn-salvar-editar-avaliacao"
);
...
btnSalvarAvaliacao.addEventListener("click", async () => {
  ...
  const questoesElements = document.querySelectorAll(
    "#container-questoes .questao:not(.questao-template)"
  );
  questoesAdicionadas = Array.from(questoesElements).map((questaoEl) => {
    ...
    const opcoes = Array.from(
      questaoEl.querySelectorAll(".opcao-container")
    ).map((opt) => opt.querySelector(".texto-opcao").value);
...
btnNovoEvento.addEventListener("click", function () {
  ...
  resetFormEvento();
  document.querySelector("#modal-evento h3").textContent =
...
const btnPaginaAnteriorAvaliacao = document.getElementById(
  "btn-page-anterior-avaliacao"
);
const btnPaginaProximoAvaliacao = document.getElementById(
  "btn-page-proximo-avaliacao"
);
...
function carregarEventos() {
  ...
  if (
    window.EventosAPI &&
    typeof window.EventosAPI.getAllAssessmentEvents === "function"
  ) {
...
function mostrarEventosNaTabela(eventos) {
  ...
  eventos.forEach((evento) => {
    ...
    tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ${
                  evento.assessmentName ||
                  (evento.assessment ? evento.assessment.name : "")
                }
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
                ${
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-blue-600 hover:text-blue-900 mr-3 exportar-avaliacao" data-id="${
                  evento.id
                }" title="Exportar Prova">
                    <i class="fas fa-file-export"></i>
                </button>
                <button class="text-yellow-600 hover:text-yellow-900 mr-3 editar-evento" data-id="${
                  evento.id
                }">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="text-red-600 hover:text-red-900 excluir-evento" data-id="${
                  evento.id
                }">
...
function configurarAdicionarQuestoes() {
  ...
  function adicionarNovaQuestao(questaoData = null) {
    ...
    if (questaoData) {
      ...
      if (
        questaoData.respostaCorreta !== undefined &&
    ...
    if (btnRemoverQuestao) {
      btnRemoverQuestao.addEventListener("click", function () {
        ...
        const questoes = document.querySelectorAll(
          "#container-questoes .questao:not(.questao-template)"
        );
    ...
    const botoesRemoverOpcao =
      novaQuestao.querySelectorAll(".btn-remover-opcao");
  ...
  function adicionarNovaOpcao(questaoEl, questaoId) {
    ...
    if (inputTexto) {
      inputTexto.value = "";
      inputTexto.placeholder = `Alternativa ${String.fromCharCode(
        65 + opcoesContainer.length
      )}`;
    }
...
function adicionarPalavrasMultiplas() {
  ...
  if (adicionadas > 0) {
    if (repetidas > 0) {
      alert(
        `${adicionadas} palavra(s) adicionada(s). ${repetidas} palavra(s) foi(ram) ignorada(s) por já existir(em).`
      );
    }
    novaPalavra.value = "";
    novaPalavra.focus();
...
function adicionarPseudopalavrasMultiplas() {
  ...
  if (adicionadas > 0) {
    if (repetidas > 0) {
      alert(
        `${adicionadas} pseudopalavra(s) adicionada(s). ${repetidas} pseudopalavra(s) foi(ram) ignorada(s) por já existir(em).`
      );
    }
...
function adicionarFrasesMultiplas() {
  ...
  if (adicionadas > 0) {
    if (repetidas > 0) {
      alert(
        `${adicionadas} frase(s) adicionada(s). ${repetidas} frase(s) foi(ram) ignorada(s) por já existir(em).`
      );
    }
    novaFrase.value = "";
    novaFrase.focus();
...
function adicionarPalavraAoDOM(palavra, isEdit = false) {
  ...
  div.querySelector("button").addEventListener("click", function () {
    ...
    if (isEdit) {
      palavrasAdicionadasEdit = palavrasAdicionadasEdit.filter(
        (p) => p !== palavraParaRemover
      );
    } else {
      palavrasAdicionadas = palavrasAdicionadas.filter(
        (p) => p !== palavraParaRemover
      );
    }
    div.remove();
  ...
  const container = isEdit
    ? document.getElementById("lista-palavras-edit")
    : listaPalavras;
...
function adicionarPseudopalavraAoDOM(pseudopalavra, isEdit = false) {
  const div = document.createElement("div");
  div.className =
    "bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm flex items-center";
  div.innerHTML = `
        ${pseudopalavra}
  ...
  div.querySelector("button").addEventListener("click", function () {
    ...
    if (isEdit) {
      pseudopalavrasAdicionadasEdit = pseudopalavrasAdicionadasEdit.filter(
        (p) => p !== pseudopalavraParaRemover
      );
    } else {
      pseudopalavrasAdicionadas = pseudopalavrasAdicionadas.filter(
        (p) => p !== pseudopalavraParaRemover
      );
    }
    div.remove();
  ...
  const container = isEdit
    ? document.getElementById("lista-pseudopalavras-edit")
    : listaPseudopalavras;
  container.appendChild(div);
}
...
function adicionarFraseAoDOM(frase, isEdit = false) {
  const div = document.createElement("div");
  div.className =
    "bg-green-100 text-green-800 px-3 py-2 rounded-md text-sm flex items-center justify-between";
  div.innerHTML = `
  ...
  div.querySelector("button").addEventListener("click", function () {
    ...
    if (isEdit) {
      frasesAdicionadasEdit = frasesAdicionadasEdit.filter(
        (f) => f !== fraseParaRemover
      );
    } else {
      frasesAdicionadas = frasesAdicionadas.filter(
        (f) => f !== fraseParaRemover
      );
    }
    div.remove();
  ...
  const container = isEdit
    ? document.getElementById("lista-frases-edit")
    : listaFrases;
  container.appendChild(div);
}
...
function resetQuestoes() {
  containerQuestoes
    .querySelectorAll(".questao:not(.questao-template)")
...
function mostrarAvaliacoesNaTabela(avaliacoes) {
  ...
  avaliacoes.forEach((avaliacao) => {
    ...
    tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ${
                  avaliacao.totalPseudowords ||
                  avaliacao.totalPseudopalavras ||
                  0
                }
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                ...
                <button class="text-yellow-600 hover:text-yellow-900 mr-3 editar-avaliacao" data-id="${
                  avaliacao.id
                }" title="Editar Avaliação">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="text-red-600 hover:text-red-900 excluir-avaliacao" data-id="${
                  avaliacao.id
...
const atualizarContadoresEdit = () => {
  const totalPalavras = document.getElementById("lista-palavras-edit").children
    .length;
  const totalPseudopalavras = document.getElementById(
    "lista-pseudopalavras-edit"
  ).children.length;
  const totalFrases =
    document.getElementById("lista-frases-edit").children.length;

  document.getElementById("contador-palavras-edit").textContent = totalPalavras;
  document.getElementById("contador-pseudopalavras-edit").textContent =
    totalPseudopalavras;
  ...
  const contadorPseudopalavras = document.getElementById(
    "contador-pseudopalavras-edit"
  );
...
function editarAvaliacao(id) {
  fetch(`${API_BASE_URL}/assessments/${id}`, {
    ...
    .then((avaliacao) => {
      ...
      document.getElementById("nome-avaliacao-edit").value =
        avaliacao.name || "";
      document.getElementById("faixa-serie-edit").value =
        avaliacao.gradeRange || "";
      document.getElementById("texto-avaliacao-edit").value =
        avaliacao.text || "";
      ...
      // Carregar questões
      const containerQuestoesEdit = document.getElementById(
        "container-questoes-edit"
      );
      ...
      if (!avaliacao.questions || avaliacao.questions.length === 0) {
        ...
        avaliacao.questions.forEach((questao) => {
          ...
          questaoDiv.className =
            "questao border rounded-md p-4 bg-gray-50 relative";
          questaoDiv.innerHTML = `
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="enunciado-questao w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" 
                            rows="2" placeholder="Digite o enunciado da questão">${
                              questao.text
                            }</textarea>
                    </div>
                          ...
                          .map(
                            ...
                            <div class="opcao-container flex items-center">
                                <input type="text" class="texto-opcao flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" 
                                    value="${opcao}" placeholder="Alternativa ${String.fromCharCode(
                              65 + index
                            )}">
          ...
          questaoDiv
            .querySelector(".btn-remover-questao")
            .addEventListener("click", function () {
              questaoDiv.remove();
              if (
                containerQuestoesEdit.querySelectorAll(".questao").length === 0
              ) {
                semQuestoesEdit.classList.remove("hidden");
              }
            });
          ...
          questaoDiv.querySelectorAll(".btn-remover-opcao").forEach((btn) => {
            btn.addEventListener("click", function () {
              ...
              if (
                opcoesContainer.querySelectorAll(".opcao-container").length > 2
              ) {
                this.closest(".opcao-container").remove();
              } else {
                alert("É necessário ter pelo menos 2 alternativas.");
          ...
          questaoDiv
            .querySelector(".btn-adicionar-opcao")
            .addEventListener("click", function () {
              ...
              novaOpcao
                .querySelector(".btn-remover-opcao")
                .addEventListener("click", function () {
                  if (
                    opcoesContainer.querySelectorAll(".opcao-container")
                      .length > 2
                  ) {
                    novaOpcao.remove();
                  } else {
...
const configurarBotoesRemoverEdit = () => {
  ...
  document
    .querySelectorAll("#lista-palavras-edit .bg-blue-100 button")
    .forEach((btn) => {
      btn.addEventListener("click", function () {
  ...
  document
    .querySelectorAll("#lista-pseudopalavras-edit .bg-purple-100 button")
    .forEach((btn) => {
      btn.addEventListener("click", function () {
  ...
  document
    .querySelectorAll("#lista-frases-edit .bg-green-100 button")
    .forEach((btn) => {
      btn.addEventListener("click", function () {
...
const adicionarPalavraEdit = () => {
  ...
  palavras.forEach((palavra) => {
    const palavrasExistentes = Array.from(listaPalavrasEdit.children).map(
...
const adicionarPseudopalavraEdit = () => {
  const novaPseudopalavraEdit = document.getElementById(
    "nova-pseudopalavra-edit"
  );
  ...
  const pseudopalavras = texto
    ...
    .map((p) => p.trim())
    .filter((p) => p !== "");
  const listaPseudopalavrasEdit = document.getElementById(
    "lista-pseudopalavras-edit"
  );

  pseudopalavras.forEach((pseudopalavra) => {
    const pseudopalavrasExistentes = Array.from(
      listaPseudopalavrasEdit.children
...
async function carregarAvaliacoesParaSelect() {
  ...
  avaliacaoSelect.innerHTML =
    '<option value="">Carregando avaliações...</option>';
  ...
  return fetch(API_BASE_URL + "/assessments", {
    ...
    .catch((error) => {
      ...
      avaliacaoSelect.innerHTML =
        '<option value="">Erro ao carregar avaliações</option>';
...
function editarEvento(id) {
  ...
  if (
    window.EventosAPI &&
    typeof window.EventosAPI.getAssessmentEventById === "function"
  ) {
    // Usar a API mais moderna
    window.EventosAPI.getAssessmentEventById(id)
      .then((evento) => {
        ...
        document.getElementById("nome-evento").value = evento.nome;
        document.getElementById("status-evento").checked =
          evento.status === "ACTIVE";

        // Carregar avaliações e selecionar a correta
        carregarAvaliacoesParaSelect().then(() => {
          document.getElementById("avaliacao-evento").value =
            evento.assessmentId;
        });
...
function excluirEvento(id) {
  if (confirm("Tem certeza que deseja excluir este evento?")) {
    ...
    if (
      window.EventosAPI &&
      typeof window.EventosAPI.deleteAssessmentEvent === "function"
    ) {
      // Usar a API mais moderna
...
async function exportarProva(id) {
  ...
  if (response.message === "Document generated successfully")
...
function adicionarQuestaoExistente(questao, isEdit = false) {
  const containerQuestoes = isEdit
    ? document.getElementById("container-questoes-edit")
    : document.getElementById("container-questoes");
  const template = isEdit
    ? document.getElementById("template-questao-edit")
    : document.getElementById("template-questao");
  ...
  novaQuestao
    ...
    .addEventListener("click", () => {
      novaQuestao.remove();
      const questoes = containerQuestoes.querySelectorAll(
        ".questao:not(.questao-template)"
      );
      if (questoes.length === 0) {
        const semQuestoes = isEdit
          ? document.getElementById("sem-questoes-edit")
          : document.getElementById("sem-questoes");
        semQuestoes.classList.remove("hidden");
      }
    });
...
document
  .getElementById("adicionar-palavra-edit")
  .addEventListener("click", adicionarPalavraEdit);
document
  .getElementById("adicionar-pseudopalavra-edit")
  .addEventListener("click", adicionarPseudopalavraEdit);
document
  .getElementById("adicionar-frase-edit")
  .addEventListener("click", adicionarFraseEdit);

// Adicionar event listeners para tecla Enter nos campos de input do modal de edição
document
  .getElementById("nova-palavra-edit")
...
document
  .getElementById("nova-pseudopalavra-edit")
  .addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
...
document
  .getElementById("nova-frase-edit")
  .addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
      adicionarFraseEdit();
...
document
  .getElementById("texto-avaliacao-edit")
  .addEventListener("input", function () {
    const texto = this.value.trim();
    const palavras = texto === "" ? 0 : texto.split(/\s+/).length;
    const contagemPalavrasEdit = document.getElementById(
      "contagem-palavras-edit"
    );
...
document
  .getElementById("adicionar-questao-edit")
  .addEventListener("click", function () {
    const containerQuestoesEdit = document.getElementById(
      "container-questoes-edit"
    );
    ...
    questaoDiv
      ...
      .addEventListener("click", function () {
        questaoDiv.remove();
        const questoes = containerQuestoesEdit.querySelectorAll(
          ".questao:not(.questao-template)"
        );
        if (questoes.length === 0) {
          semQuestoesEdit.classList.remove("hidden");
...
btnSalvarEditarAvaliacao.addEventListener("click", async () => {
  ...
  const questoesElements = document.querySelectorAll(
    "#container-questoes-edit .questao:not(.questao-template)"
  );
  const questoesAdicionadas = Array.from(questoesElements)
    .map((questaoEl) => {
      const enunciado = questaoEl.querySelector(".enunciado-questao").value;
      const opcoes = Array.from(questaoEl.querySelectorAll(".opcao-container"))
        .map((opt) => opt.querySelector(".texto-opcao").value)
  ...
  const request = {
    ...
    text: texto,
    words: Array.from(
      document.getElementById("lista-palavras-edit").children
    ).map((el) => el.textContent.trim().replace(/[×✕]$/, "").trim()),
    pseudowords: Array.from(
      document.getElementById("lista-pseudopalavras-edit").children
    ).map((el) => el.textContent.trim().replace(/[×✕]$/, "").trim()),
    phrases: Array.from(
      document.getElementById("lista-frases-edit").children
    ).map((el) => ({
    ...
    totalPseudowords: document.getElementById("lista-pseudopalavras-edit")
      .children.length,
  };
